...
# Usamos la configuración más simple posible para evitar el error 404
...
with col2:
    ...
    if f2:
        try:
            # Saltamos 3 líneas y usamos ; como vimos en tus fotos
            df2 = pd.read_csv(f2, sep=';', encoding='latin1', skiprows=3)
            # Guardamos solo lo vital para no saturar la memoria
...
if st.session_state.db_hospital and st.session_state.db_cenabast:
    ...
    if st.button("🚀 Iniciar Cruce de Bases de Datos"):
        with st.spinner("La IA está razonando las equivalencias comerciales..."):
            ...
            # El "Cerebro" recibe ambas bases de datos y busca las marcas
            prompt = f"""
            Actúa como Jefe de Farmacia. Cruza estas dos bases de datos internas:
            
            DATOS HOSPITAL:
            {st.session_state.db_hospital[:12000]}
            
            DATOS CENABAST:
            {st.session_state.db_cenabast[:12000]}
            
            TAREA:
            1. Busca qué productos del hospital tienen Saldo Meses bajo.
            2. Busca el 'NOMBRE COMERCIAL' equivalente en CENABAST (Ej: AAS = Aspirina/Sustitutos).
            3. Haz una tabla con: Producto | Marca CENABAST | Estado.
            4. Reporta si hay 'SUSPENSION POR DEUDA'.
            """
            
            try:
                # Llamada de contingencia: si falla, intentamos una vía alternativa
                response = model.generate_content(prompt)
                st.markdown("### 📋 Resultados del Análisis")
                st.markdown(response.text)
            except Exception as e:
                st.error("⚠️ Error de conexión con la IA. Intenta refrescar o actualizar el paquete 'google-generativeai'.")
